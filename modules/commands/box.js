...
`,
	...
	if (args[0] == 'me') {
		if (args[1] == 'admin') {
			...
			const find = threadInfo.adminIDs.find(
				el => el.id == api.getCurrentUserID()
			);
			if (!find)
				api.sendMessage(
					'BOT cần ném quản trị viên để dùng ?',
					event.threadID,
					event.messageID
				);
	...
	if (args[0] == 'admin') {
		...
		if (!finddd)
			return api.sendMessage(
				'Mày đéo phải quản trị viên box ?',
				event.threadID,
				event.messageID
			);
		if (!find) {
			api.sendMessage(
				'Không ném quản trị viên dùng con cặc ?',
				event.threadID,
				event.messageID
			);
		}
	...
	if (args[0] == 'image') {
		if (event.type !== 'message_reply')
			return api.sendMessage(
				'❌ Bạn phải reply một audio, video, ảnh nào đó',
				event.threadID,
				event.messageID
			);
		if (
		...
		if (event.messageReply.attachments.length > 1)
			return api.sendMessage(
				`Vui lòng reply chỉ một audio, video, ảnh!`,
				event.threadID,
				event.messageID
			);
		var callback = () =>
